...
RSpec.describe BrainzLab::Configuration do
  ...
  describe "defaults" do
    it "has default recall settings" do
      expect(config.recall_enabled).to eq(true)
    ...
    it "has default reflex settings" do
      expect(config.reflex_enabled).to eq(true)
  ...
  describe "#level_enabled?" do
    it "returns true for levels at or above min" do
      ...
      expect(config.level_enabled?(:debug)).to eq(false)
      expect(config.level_enabled?(:info)).to eq(false)
      expect(config.level_enabled?(:warn)).to eq(true)
      expect(config.level_enabled?(:error)).to eq(true)
      expect(config.level_enabled?(:fatal)).to eq(true)
    end
  end
