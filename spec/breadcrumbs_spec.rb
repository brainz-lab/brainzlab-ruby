...
RSpec.describe BrainzLab::Reflex::Breadcrumbs do
  ...
  describe '#add' do
    ...
    it 'converts message to string' do
      breadcrumbs.add(message: 12_345)
...
RSpec.describe BrainzLab::Reflex, '.breadcrumbs' do
  ...
  it 'adds breadcrumb to current context' do
    described_class.add_breadcrumb('Test message', category: 'test', level: :debug, data: { foo: 'bar' })

    crumbs = described_class.breadcrumbs.to_a
    expect(crumbs.size).to eq(1)
  ...
  it 'clears breadcrumbs from current context' do
    described_class.add_breadcrumb('Test')
    expect(described_class.breadcrumbs.size).to eq(1)

    described_class.clear_breadcrumbs!
    expect(described_class.breadcrumbs.size).to eq(0)
  end
end
